...
def _decay_factor(age_hours):
    """指数衰减系数"""
    if age_hours < 24:
        return 1.0 - (age_hours / 24) * 0.3
    if age_hours < 168:  # 7天
        return 0.7 - ((age_hours - 24) / 144) * 0.3
    if age_hours < 720:  # 30天
        return 0.4 - ((age_hours - 168) / 552) * 0.25
    return 0.15 - min((age_hours - 720) / 2160, 0.1)
...
""")
    ...
    for i, mem in enumerate(memories):
        ...
        # 计算相对时间描述
        time_desc = _describe_age(today, mem["timestamp"])
