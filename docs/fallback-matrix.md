...
> **版本**: v4.0.1 | **覆盖率**: 7/7 组件 | **保障**: 核心功能任何条件下均可用
>
> **P0-1 更新（v4.0.1）**：能量推断组件的主路径已调整为**默认关键词分类**（零网络依赖、亚毫秒级），
> LLM 推断改为 opt-in（`enable_llm_energy=True`）。这确保 `add()` 主路径在任何网络条件下都不阻塞。
...
| **能量推断** | 关键词规则 (默认, 零网络) | LLM (opt-in, ≥85%) | 默认值 | — |
