...
class DiZhi:
    ...
    # 六合关系（地支索引对）：子丑合, 寅亥合, 卯戌合, 辰酉合, 巳申合, 午未合
    LIUHE = {0: 1, 1: 0, 2: 11, 11: 2, 3: 10, 10: 3, 4: 9, 9: 4, 5: 8, 8: 5, 6: 7, 7: 6}
    # 三合局（三个地支组成一局）：申子辰(水), 亥卯未(木), 寅午戌(火), 巳酉丑(金)
    SANHE = [
        {8, 0, 4},   # 申子辰 → 水局
        {11, 3, 7},  # 亥卯未 → 木局
        {2, 6, 10},  # 寅午戌 → 火局
...
class TemporalInfo:
    ...
    tian_gan: str        # 天干
    di_zhi: str          # 地支
    ganzhi: str           # 完整干支（如"甲子"）
    wuxing: str           # 五行
    yin_yang: str         # 阴阳
    season: str           # 季节
    is_birthday: bool     # 是否旺日
...
class TemporalSystem:
    ...
    def _init_60_jiazi(self):
        """初始化60甲子表"""
        # 甲子从1984年开始，每60年一循环
        base_year = self._JIAZI_CYCLE_START
        ...
        # 计算年的60甲子
        current_ganzhi_idx = 0  # 1984 = 甲子 = 索引0
        for year in range(1924, 2044):  # 覆盖近百年
            if year == base_year:
                current_ganzhi_idx = 0
            else:
                current_ganzhi_idx = (year - base_year) % 60
            
            self._year_ganzhi[year] = self._get_ganzhi_name(current_ganzhi_idx)
    
    def _get_ganzhi_name(self, idx: int) -> str:
        """获取干支名称"""
        tg_idx = idx % 10
        dz_idx = idx % 12
        return TianGan.NAMES[tg_idx] + DiZhi.NAMES[dz_idx]
    
    def date_to_ganzhi(self, dt: date) -> TemporalInfo:
        """
        将日期转换为干支信息
        ...
        Returns:
            TemporalInfo: 包含年干支、月干支、日干支、时干支
        """
        year = dt.year
        month = dt.month
        day = dt.day
        ...
        # 年干支
        year_gan = self._year_ganzhi.get(year, self._guess_year_ganzhi(year))
        
        # 日干支（简化计算，实际需要查表或天文计算）
        day_gan, day_zhi = self._calc_day_ganzhi(dt)
        ...
        return TemporalInfo(
            tian_gan=year_gan[0],
            di_zhi=day_zhi,
            ganzhi=year_gan,  # 主干支用年柱
            wuxing=DiZhi.WUXING[dz_idx],
            yin_yang="阳" if DiZhi.YIN_YANG[dz_idx] else "阴",
            season=season,
            is_birthday=self._is_wang_day(tg_idx, dz_idx)
        )
    
    def _guess_year_ganzhi(self, year: int) -> str:
        """推测年干支"""
        idx = (year - self._JIAZI_CYCLE_START) % 60
        return self._get_ganzhi_name(idx)
    ...
    def _calc_day_ganzhi(self, dt: date) -> Tuple[str, str]:
        ...
        return self._get_ganzhi_name(idx), DiZhi.NAMES[idx % 12]
    ...
    def get_monthly_wuxing_state(self, month: int, day: int = 15) -> Dict[str, str]:
        """
        获取当前月令下各五行的旺衰状态
        
        Returns:
            {"木": "旺", "火": "相", "土": "休", "金": "囚", "水": "死"}  # 春季示例
        """
        season = self._get_season(month, day)
    ...
    def _is_wang_day(self, tg_idx: int, dz_idx: int) -> bool:
        """判断是否旺日（简化判断：同五行则旺）"""
        tg_wuxing = TianGan.WUXING[tg_idx]
        dz_wuxing = DiZhi.WUXING[dz_idx]
        return tg_wuxing == dz_wuxing
    
    def get_jiazi_position(self, dt: date) -> int:
        """
        获取日期在六十甲子循环中的位置（0-59）
    ...
    def calculate_time_decay(self, memory_timestamp: int, memory_wuxing: str) -> float:
        """
        计算记忆的时间衰减因子（0.0~1.0）
        
        公式：decay = base_decay * ganzhi_multiplier
        ...
        - ganzhi_multiplier: 干支调制因子（基于月令旺衰）
          - 记忆五行当令（旺）: ×1.3
          - 记忆五行相（次旺）: ×1.15
          - 记忆五行休: ×1.0
          - 记忆五行囚: ×0.8
        ...
        # 干支调制因子（基于当前月令旺衰）
        today = date.today()
        monthly_states = self.get_monthly_wuxing_state(today.month, today.day)
        state = monthly_states.get(memory_wuxing, "休")
    ...
    def calculate_priority(
        ...
        # 月令旺相加成（替代简单 season_boost）
        today = date.today()
        monthly_states = self.get_monthly_wuxing_state(today.month, today.day)
        wuxing_state = monthly_states.get(memory_wuxing, "休")
        ...
        season_boost = state_boost_map.get(wuxing_state, 0.0)
        
        # 旺日加成
        wang_boost = 0.1 if ganzhi_info.is_birthday else 0.0
        
        # 阴阳加成
        yin_boost = 0.05 if ganzhi_info.yin_yang == "阴" else 0.0
        
        # 时间衰减（如果提供了 timestamp）
        time_decay_factor = 1.0
        if memory_timestamp is not None:
            time_decay_factor = self.calculate_time_decay(memory_timestamp, memory_wuxing)
    ...
    def get_current_ganzhi(self) -> TemporalInfo:
        """获取当前时刻的干支信息"""
        return self.date_to_ganzhi(date.today())
