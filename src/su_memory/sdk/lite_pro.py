...
class SuMemoryLitePro:
    ...
    def __init__(
        ...
        **embedding_kwargs
        ...
        if enable_vector:
            ...
            if ollama_available:
                try:
                    ...
                    print(f"[SuMemoryLitePro] 使用 Ollama bge-m3 向量服务")
                except Exception as e:
                    print(f"[SuMemoryLitePro] Ollama 初始化失败: {e}")
            
            # 2. 如果没有 Ollama，尝试用户指定的后端
            if not self._embedding and embedding_backend != "minimax":
                try:
                    self._embedding = EmbeddingManager(embedding_backend, **embedding_kwargs)
                    self._embedding_backend_type = embedding_backend
                except Exception as e:
                    print(f"[SuMemoryLitePro] Embedding init failed: {e}")
            
            # 3. 最后尝试 sentence-transformers
            if not self._embedding:
                try:
                    ...
                    print(f"[SuMemoryLitePro] 使用 sentence-transformers")
                except Exception as e:
                    print(f"[SuMemoryLitePro] sentence-transformers 不可用: {e}")
                    print(f"[SuMemoryLitePro] 警告: 无向量服务，仅使用关键词检索")
        ...
        if enable_vector and FAISS_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] FAISS 索引创建失败: {e}")
        ...
        if enable_graph and VECTOR_GRAPH_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] VectorGraphRAG 多跳推理引擎已初始化 (FAISS enabled)")
            except Exception as e:
        ...
        if enable_temporal and SPACETIME_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] SpacetimeIndex 时空索引已初始化")
            except Exception as e:
        ...
        if SPACETIME_MULTIHOP_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] SpacetimeMultihopEngine 时空多跳融合引擎已初始化")
            except Exception as e:
        ...
        if MULTIMODAL_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] MultimodalEmbedding 多模态管理器已初始化")
            except Exception as e:
        ...
        if SPATIAL_RAG_AVAILABLE and self._embedding:
            try:
                ...
                print(f"[SuMemoryLitePro] SpatialRAG 三维世界模型已初始化")
            except Exception as e:
    ...
    def _vector_search(self, query: str, top_k: int = 20) -> List[Dict]:
        ...
        # 优先使用 FAISS 索引
        if self._faiss_index and self._id_faiss_map:
    ...
    def _faiss_vector_search(self, query_vec: List[float], top_k: int = 20) -> List[Dict]:
        ...
        try:
            ...
            # 搜索
            D, I = self._faiss_index.search(query_np, min(top_k * 2, len(self._id_faiss_map)))
