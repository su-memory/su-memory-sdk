...
def _seed_lab_references() -> list[LabReference]:
    return [
        LabReference("白蛋白", "g/L", 35, 55, 30, None, "低白蛋白提示营养不良/肝病/肾病综合征"),
        LabReference("前白蛋白", "mg/L", 200, 400, 100, None, "半衰期短，反映近期营养状况"),
        LabReference("血红蛋白", "g/L", 120, 160, 70, None, "贫血分级：<90中度 <70重度（男）"),
        LabReference("钾", "mmol/L", 3.5, 5.5, 3.0, 6.0, "高钾>6.0危及生命，低钾<3.0需紧急补钾"),
        LabReference("钠", "mmol/L", 135, 145, 125, 155, "低钠<135需限水，高钠>145需补水"),
        LabReference("血糖", "mmol/L", 3.9, 6.1, 2.8, 16.7, "空腹血糖；>7.0糖尿病，<3.9低血糖"),
        LabReference("肌酐", "μmol/L", 44, 133, None, None, "反映肾功能；eGFR更准确"),
        LabReference("C反应蛋白", "mg/L", 0, 10, None, 100, "炎症标志；>100提示严重感染/炎症"),
        LabReference("转铁蛋白", "g/L", 2.0, 3.6, 1.5, None, "营养+铁代谢指标"),
        LabReference("BMI", "kg/m²", 18.5, 24.0, 17.0, 32.0, "<18.5偏瘦 <17严重营养不良 >28肥胖"),
    ]
...
def _seed_allergies() -> list[AllergyEntry]:
    return [
        AllergyEntry(
            "花生",
            ["花生蛋白", "花生油", "花生酱", "arachis_oil"],
            "major",
            "改用其他植物油/蛋白来源",
        ),
        AllergyEntry(
            "牛奶", ["乳糖", "乳清蛋白", "酪蛋白", "whey"], "moderate", "选用无乳糖配方或植物蛋白"
        ),
        AllergyEntry(
            "鸡蛋", ["卵白蛋白", "蛋清", "ovalbumin", "lysozyme"], "moderate", "选用无蛋配方"
        ),
        AllergyEntry(
            "麸质", ["小麦蛋白", "大麦", "黑麦", "gluten", "麦胶"], "moderate", "选用无麸质配方"
        ),
        AllergyEntry(
            "大豆",
            ["大豆蛋白", "豆粕", "soy_protein", "大豆卵磷脂"],
            "moderate",
            "选用其他植物蛋白",
        ),
        AllergyEntry(
            "鱼", ["鱼精蛋白", "鱼油", "omega3_fish"], "moderate", "改用亚麻籽油(植物omega3)"
        ),
    ]
...
class MedicalKnowledgeBase:
    ...
    def __init__(self):
        ...
        self._allergies: dict[str, AllergyEntry] = {ae.allergen: ae for ae in _seed_allergies()}
    ...
    def load_from_file(cls, path: str) -> MedicalKnowledgeBase:
        ...
        if "drug_interactions" in data:
            kb._drug_interactions = [
                _drug_interaction_from_dict(d) for d in data["drug_interactions"]
            ]
        if "lab_references" in data:
            kb._lab_references = {
                lr.name: lr for lr in (_lab_reference_from_dict(d) for d in data["lab_references"])
            }
        if "allergies" in data:
            kb._allergies = {
                ae.allergen: ae for ae in (_allergy_from_dict(d) for d in data["allergies"])
            }
        return kb
    ...
    def get_contraindicated_nutrients(self, drug_list: list[str]) -> dict[str, list[str]]:
        """获取药物列表的所有禁忌营养素。
    ...
    def find_abnormal_from_dict(self, labs: dict[str, float]) -> list[dict[str, Any]]:
        """从检验值字典中找出异常项。
    ...
    def check_substance_allergy(self, substances: list[str]) -> list[AllergyEntry]:
        """检查物质列表中是否有已知过敏原。
