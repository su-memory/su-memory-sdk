...
def _seed_rules() -> list[AssociationRule]:
    ...
    drug_nutrient_pairs = [
        ...
        (
            "captopril",
            ["卡托普利", "captopril", "ACE抑制剂", "ACEI"],
            ["钾", "potassium", "高钾"],
            "ACE抑制剂减少钾排泄，需避免高钾饮食",
        ),
        (
            "ciprofloxacin",
            ["环丙沙星", "ciprofloxacin", "喹诺酮"],
        ...
        (
            "levothyroxine",
            ["左旋甲状腺素", "levothyroxine", "甲状腺素"],
            ["钙", "calcium", "铁", "iron", "大豆", "黄豆"],
            "甲状腺素与钙/铁/大豆结合影响吸收，空腹服用",
        ),
        (
            "diuretic",
            ["利尿剂", "diuretic", "呋塞米", "furosemide"],
    ...
    deficiency_pairs = [
        (
            "protein",
            ["低蛋白", "蛋白质缺乏", "hypoalbuminemia", "白蛋白低", "白蛋白偏低"],
            ["水肿", "edema", "营养不良", "愈合延迟", "感染"],
            "蛋白质缺乏导致低蛋白血症、水肿和愈合延迟",
        ),
        (
            "iron",
            ["缺铁", "iron_deficiency", "铁缺乏"],
            ["贫血", "anemia", "乏力", "面色苍白"],
            "缺铁性贫血表现为乏力、面色苍白",
        ),
        (
            "vitd",
            ["维生素D缺乏", "vitamin_d_deficiency"],
        ...
        (
            "b1",
            ["维生素B1缺乏", "thiamine_deficiency", "脚气病"],
    ...
    allergy_pairs = [
        (
            "peanut",
            ["花生过敏", "peanut_allergy", "花生"],
            ["花生蛋白", "花生油", "peanut_protein", "坚果"],
            "花生过敏者禁用含花生蛋白/花生油的制剂",
        ),
        (
            "lactose",
            ["乳糖不耐受", "lactose_intolerance"],
            ["乳糖", "牛奶", "乳清蛋白", "lactose"],
            "乳糖不耐受者避免含乳糖制剂，可选无乳糖配方",
        ),
        (
            "gluten",
            ["麸质过敏", "乳糜泻", "celiac", "gluten"],
            ["小麦", "大麦", "麸质", "wheat", "gluten"],
            "麸质过敏/乳糜泻患者避免含麸质食物",
        ),
        (
            "egg",
            ["鸡蛋过敏", "egg_allergy", "蛋清过敏"],
    ...
    disease_pairs = [
        (
            "ckd",
            ["慢性肾病", "CKD", "肾功能不全", "chronic_kidney"],
            ["低蛋白饮食", "low_protein", "优质蛋白", "限钾", "限磷"],
            "慢性肾病需低蛋白饮食，限制钾磷摄入",
        ),
        (
            "diabetes",
            ["糖尿病", "diabetes", "高血糖", "diabetic"],
...
class MedicalAssociationKB:
    ...
    def inject_hooks(self, client: SuMemoryLitePro) -> None:
        ...
        logger.info("[MedicalKB] 已注入医疗关联钩子，规则数=%d", len(self._rules))
    ...
    def _scan_and_link(self, new_memory_id: str, new_content: str) -> int:
        ...
        for mem_id, node in graph._nodes.items():
            ...
            for rule in new_sources:
                if rule in exist_targets:
                    self._create_link(client, new_memory_id, mem_id, rule)
                    linked_count += 1

            # 新内容是 target 端，已有内容是 source 端
            for rule in new_targets:
                if rule in exist_sources:
                    self._create_link(client, mem_id, new_memory_id, rule)
                    linked_count += 1
    ...
    def explain_association(self, source_content: str, target_content: str) -> str | None:
