...
class RecencyFeedbackSystem:
    ...
    在 TemporalSystem 基础上叠加用户反馈调节：
        ...
        base_score = temporal_system.calculate_time_decay(memory_timestamp, energy_type)
    ...
    使用方法：
        ...
        # 计算最终得分
        score = feedback_sys.calculate_recency_score("mem-001", mem_timestamp, energy_type)
    """
    ...
    def calculate_recency_score(
        ...
        Args:
            ...
            memory_energy_type: 记忆的energy_type属性（影响衰减速度）
        ...
        base_decay = self._temporal.calculate_time_decay(memory_timestamp, memory_energy_type)

        # 2. 近期性 boost（记忆越新 boost 越高，防止新记忆被旧记忆压制）
        recency_boost = self._temporal.calculate_recency_boost(
            memory_timestamp, q_ts, memory_energy_type
        )
